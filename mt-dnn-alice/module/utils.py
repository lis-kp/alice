...
def mean_teacher_criterion(logit_stu, logit_tea, class_name, _lambda, temperature=1):
    ...
    if class_name == "prob":
        ...
        r_stu = -(1/(prob_stu + EPSILON) -1 + EPSILON).detach().log()
        r_tea = -(1/(prob_tea + EPSILON) -1 + EPSILON).detach().log()
        ...
        logprob_stu = F.log_softmax(logit_stu/temperature, 1)
        prob_tea = F.log_softmax(logit_tea/temperature, 1).exp()
        return -(prob_tea * logprob_stu).sum(-1).mean() * _lambda
