...
def get_lemmas(sentence):
    # get the lemmas of the sentence using the MeCab tagger
    lemmas = []
    for line in mecab.parse(sentence).splitlines()[:-1]:
        features = line.split('\t')
        lemma = features[3]
        lemmas.append(lemma)
#        print(lemmas)
...
def bert_feature_extractor(
        text_a, text_b=None, max_seq_length=512, tokenize_fn=None):
    #get lemmas
    text_a = get_lemmas(text_a)

    tokens_a = tokenize_fn.tokenize(text_a)
    tokens_b = None

    if text_b:
        text_b = get_lemmas(text_b)
    ...
    if tokens_b:
        ...
        input_ids = convert_tokens_to_ids(vocab, ['[CLS]'] + tokens_b + ['[SEP]'] + tokens_a + ['[SEP]'])
        ...
        input_ids = convert_tokens_to_ids(vocab, ['[CLS]'] + tokens_a + ['[SEP]'])
