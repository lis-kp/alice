...
from module.lookahead import Lookahead
from module.bert_optim import Adamax, RAdam
from module.my_optim import EMA, MeanTeacher, VirtualTeacher
from module.utils import mean_teacher_criterion
...
class MTDNNModel(object):
    def __init__(self, opt, state_dict=None, num_train_step=-1):
        ...
        #----------------------------
        if opt.get('lookahead', False):
            print('use lookahead')
            self.optimizer = Lookahead(self.optimizer, k=opt['lookahead_k'], alpha=opt['lookahead_alpha'])
        #----------------------------
        ...
        if opt['mean_teacher']:
            self.mean_teacher = MeanTeacher(self.network,
                                            opt['mean_teacher_rampup'],
                                            opt['mean_teacher_alpha1'],
                                            opt['mean_teacher_alpha2'],
                                            average=opt['mean_teacher_avg'],
                                            multi_gpu_on=opt['multi_gpu_on'])
        if opt['virtual_teacher']:
            self.virtual_teacher = VirtualTeacher(self.network,
                                                  opt['noisycopy_eps'],
                                                  opt['advcopy_eps'],
                                                  eps=opt['vat_stable_eps'],
                                                  multi_gpu_on=opt['multi_gpu_on'])
        #if opt['virtual_teacher_new_alg']:
        #    self.virtual_teacher = VirtualTeacher(self.network,
        #                                          opt['noisycopy_eps'],
        #                                          opt['advcopy_eps'],
        #                                          eps=opt['vat_stable_eps'],
    ...
    def update(self, batch_meta, batch_data):
        ...
        token_id = batch_data[batch_meta['token_id']]
        segment_id = batch_data[batch_meta['segment_id']]
        ...
        if self.config['mean_teacher']:
            teacher_logits = self.mean_teacher.forward(inputs)
            if batch_meta['pairwise']:
                teacher_logits = teacher_logits.view(-1, batch_meta['pairwise_size'])

            if task_type == TaskType.Regression:
                mt_loss = mean_teacher_criterion(logits, teacher_logits, 'logit', 1)
            else:
                mt_loss = mean_teacher_criterion(logits, teacher_logits, 'smart', 1, temperature=self.config['mt_kd_tau'])
            loss = loss + mt_loss

        if self.config['virtual_teacher']:
            vat_layer = self.config['vat_max_layer']
            vat_opt = self.config['vat_opt']
            if self.config['encoder_type'] == EncoderModelType.ROBERTA:
                mask = token_id.eq(1)
            if vat_opt == 1:
                choices = [-1] if vat_layer == -1 else [-1] + list(range(0, vat_layer + 1))
                vat_layer = random.choice(choices)
            vat_args = [token_id, segment_id, mask, premise_mask, hypothesis_mask, task_id, 1, None, vat_layer]
            embed = self.network(*vat_args)
            # add noise
            vat_args = [token_id, segment_id, mask, premise_mask, hypothesis_mask, task_id, 2, embed, vat_layer]
            logits_vat = self.mnetwork(*vat_args)
            if batch_meta['pairwise']:
                logits_vat = logits_vat.view(-1, batch_meta['pairwise_size'])
            #vat_loss = mean_teacher_criterion(logits_vat, logits.detach(), 'smart', 1)
            vat_loss = F.cross_entropy(logits_vat, y)
            vat_loss.backward()
            norm = embed.grad.norm()
            if (torch.isnan(norm) or torch.isinf(norm)):
                print("Hit nan gradient in embed vat")
                self.mnetwork.zero_grad()
                return

            adv_direct = embed.grad / (embed.grad.abs().max(-1,keepdim=True)[0] + self.config['vat_stable_eps'])
            embed = embed + adv_direct * self.config['vat_eps']
            embed = embed.detach()
            vat_args = [token_id, segment_id, mask, premise_mask, hypothesis_mask, task_id, 2, embed, vat_layer]

            # new noisy
            if self.config['use_advcopy']:
                is_nan = self.virtual_teacher.update(self.mnetwork)
                if is_nan:
                    self.mnetwork.zero_grad()
                    return
                logits_vat = self.virtual_teacher.model.embed_forward(*vat_args)
            else:
                logits_vat = self.mnetwork(*vat_args)
            ...
            if task_type == TaskType.Regression:
                vat_loss_f = mean_teacher_criterion(logits_vat, logits.detach(), 'logit', self.config['vat_lambda'])
                vat_loss_b = mean_teacher_criterion(logits, logits_vat.detach(), 'logit', self.config['vat_lambda'])
            else:
                vat_loss_f = mean_teacher_criterion(logits_vat, logits.detach(), 'smart', self.config['vat_lambda'])
                vat_loss_b = mean_teacher_criterion(logits, logits_vat.detach(), 'smart', self.config['vat_lambda'])
            y_loss = F.cross_entropy(logits_vat, y)
            vat_loss = (vat_loss_f + vat_loss_b ) * self.config.get('vat_alpha', 1)
            loss = loss + vat_loss + y_loss
            # compute y_loss
            if self.config['virtual_teacher']:
                vat_layer = self.config['vat_max_layer']
                vat_opt = self.config['vat_opt']
        ...
        #===========================================================================
        # compute y_loss
        #if self.config['virtual_teacher_new_alg']:
        #            vat_layer = self.config['vat_max_layer']
        #            vat_opt = self.config['vat_opt']
        #            if self.config['encoder_type'] == EncoderModelType.ROBERTA:
        #                mask = token_id.eq(1)
        ...
        #                self.mnetwork.zero_grad()
        #                return
        #            adv_direct = embed.grad / (
        #                        embed.grad.abs().max(-1, keepdim=True)[0] + self.config['vat_stable_eps'])
        #            embed = embed + adv_direct * self.config['vat_eps']
        #            embed = embed.detach()
        #            vat_args = [token_id, segment_id, mask, premise_mask, hypothesis_mask, task_id, 2, embed, vat_layer]
        #            # new noisy
